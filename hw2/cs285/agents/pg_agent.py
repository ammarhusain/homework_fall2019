...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rews_list):
        ...
        if not self.reward_to_go:
            ...
            # Estimate the Q value Q^{pi}(s_t, a_t) using rewards from that entire trajectory
        ...
        # Case 2: reward-to-go PG
            ...
            # Estimate the Q value Q^{pi}(s_t, a_t) as the reward-to-go
    ...
    def _discounted_return(self, rewards):
        ...
        indices = [idx for idx in range(len(rewards))]

        # 2) create a list where the entry at each index (t') is gamma^(t')
        discounts = [pow(self.gamma, idx) for idx in indices]

        # 3) create a list where the entry at each index (t') is gamma^(t') * r_{t'}
        discounted_rewards = [discounts[idx]*rewards[idx] for idx in indices]
        ...
        list_of_discounted_returns = [sum_of_discounted_rewards for idx in indices]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)):
            ...
            indices = [idx for idx in range(start_time_index, len(rewards))]

            # 2) create a list where the entry at each index (t') is gamma^(t'-t)
            discounts = [pow(self.gamma, idx - start_time_index) for idx in indices]
            ...
            discounted_rtg = [discounts[idx-start_time_index]*rewards[idx] for idx in indices]
